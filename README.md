...
> [!WARNING]
> ### ⚠️ ПРОЕКТ УСТАРЕЛ И ЗАКРЫТ В ПОЛЬЗУ [RustCooling](https://github.com/Qyzom/RustCooling)
> Данная реализация на C# (.NET / WPF) признана неэффективной, нестабильной и содержащей массу костылей (закрытые драйверы ядра WinRing0, высокий оверхед рантайма, утечки памяти и крайне нестабильная работа под Linux).
>
> Проект был полностью переписан с нуля на **Rust** в виде ультралегкого CLI и фонового демона: **[RustCooling](https://github.com/Qyzom/RustCooling)**.
> - **Потребление RAM:** менее **2 МБ** (вместо 50–80 МБ здесь).
> - **Размер:** один автономный бинарник ~750 КБ без зависимостей.
> - **Стабильность:** 100% открытый код без сторонних драйверов, нативный опрос датчиков и идеальная поддержка Linux (`systemd`, `udev`, `/sys/class/hwmon`).
>
> **Настоятельно рекомендуется использовать [RustCooling](https://github.com/Qyzom/RustCooling) — особенно если вы используете Linux!**
